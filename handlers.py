...
def get_main_keyboard():
    keyboard_layout = [
        ['Buy Number', 'Check Balance'],
        ['Show Messages', 'Cancel Order']
    ]
...
def handle_choice(update: Update, context: CallbackContext) -> int:
    ...
    if user_choice == 'buy number':
        ...
        if countries:
            ...
            country_keyboard = chunk_list(country_names, 3)
            update.message.reply_text(
                'Please select a country:',
                reply_markup=ReplyKeyboardMarkup(country_keyboard, resize_keyboard=True, one_time_keyboard=True)
            )
            logging.info(f"Presenting countries to user. First few: {country_keyboard[:2]}")
