...
def handle_choice(update: Update, context: CallbackContext) -> int:
    ...
    if user_choice == 'buy number':
        ...
        if countries:
            countries_list = [countries[country]['text_en'] for country in countries]
            country_keyboard = [countries_list[i:i + 3] for i in range(0, len(countries_list), 3)]
            update.message.reply_text(
...
def select_country(update: Update, context: CallbackContext) -> int:
    ...
    if country_code is None:
        ...
        countries_list = [countries[c]['text_en'] for c in countries]
        country_keyboard = [countries_list[i:i + 3] for i in range(0, len(countries_list), 3)]
