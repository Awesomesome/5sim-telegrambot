...
def handle_choice(update: Update, context: CallbackContext) -> int:
    ...
    if user_choice == 'buy number':
        ...
        if countries:
            country_names = [countries[country]['text_en'] for country in countries]
            country_keyboard = [country_names[i:i+2] for i in range(0, len(country_names), 2)]
            update.message.reply_text(
...
def select_country(update: Update, context: CallbackContext) -> int:
    ...
    if country_code is None:
        ...
        country_names = [countries[c]['text_en'] for c in countries]
        fallback_keyboard = [country_names[i:i+2] for i in range(0, len(country_names), 2)]
        update.message.reply_text('Invalid country selection. Please choose from the provided options.', 
                                  reply_markup=ReplyKeyboardMarkup(fallback_keyboard, one_time_keyboard=True, resize_keyboard=True))
        return SELECTING_COUNTRY
    ...
    if prices and country_code in prices:
        ...
        product_keyboard = [products[i:i+2] for i in range(0, len(products), 2)]
        update.message.reply_text(
...
def select_product(update: Update, context: CallbackContext) -> int:
    ...
    if prices and country in prices and product in prices[country]:
        ...
        operator_keyboard = [operators[i:i+2] for i in range(0, len(operators), 2)]
