...
def handle_choice(update: Update, context: CallbackContext) -> int:
    ...
    if user_choice.startswith('buy number'):
        ...
        if countries:
            ...
            return CHOOSING
    elif user_choice.startswith('check balance'):
        return check_balance(update, context)
    elif user_choice.startswith('show messages'):
        return show_messages(update, context)
    elif user_choice.startswith('cancel order'):
