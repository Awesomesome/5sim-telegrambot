...
def handle_choice(update: Update, context: CallbackContext) -> int:
    ...
    if user_choice == 'buy number':
        ...
        if countries:
            flat_countries = [countries[country]['text_en'] for country in countries]
            country_keyboard = chunk_list(flat_countries, 3)
            update.message.reply_text(
...
def select_country(update: Update, context: CallbackContext) -> int:
    ...
    if country_code is None:
        ...
        flat_countries = [countries[c]['text_en'] for c in countries]
        update.message.reply_text('Invalid country selection. Please choose from the provided options.', 
                                  reply_markup=ReplyKeyboardMarkup(chunk_list(flat_countries, 3), resize_keyboard=True, one_time_keyboard=True))
        return SELECTING_COUNTRY
